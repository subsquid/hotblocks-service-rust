...
entry point.
Phase 3 adds the observability corpus: a scripted source the test steps through one
alarm condition at a time, a `/metrics` scraper every assertion reads through, the
CT-4 truncation cases, and CT-5's surface sweep plus its upstream-counter run against
...
| INV-4 | CT-4/7 | P | CT-7's deterministic entry point drives the real adapter through a catch-up that starts below the upstream finalized head and asserts the committed buffer ends inside the window with auto-adjust off. CT-4 now drives the over-window branch itself: the level and its magnitude rise together while finality lags, and the commit that finalizes past the excess clears both. The auto-adjust soak is still absent |
...
| INV-27 | CT-4/9 | P | the empty-backfill boundary is covered; CT-4 now drives both mid-response failures — a continuity break and an exhausted acquisition after a prefix — and asserts the body ends whole on a record boundary while the failure is counted server-side. The structure-aware fuzz of CT-9 is absent |
...
| INV-30 | CT-5 | P | CT-5 pins the exposed family set to IB-12 exactly, so neither a dropped signal nor a new dead gauge passes; the worker gauge ADR-3 emptied is gone. Reconciling the state levels against the simulator ledger (HC-2) remains |
| INV-31 | CT-4 | P | CT-4 induces over-window, force advance, integrity violation, fork rebase, the `error` and `fork` restart causes, stall and terminal, and reads each back off `/metrics`. Two are asserted off the text exposition rather than the route — the watermark regression, which is driven through `init` directly, and CT-5's acquisition-retry exhaustion. The `reset` restart cause is unasserted: reaching a T1 re-seed through the ladder costs `P-STALL-REINIT` sessions of `P-SESSION-BACKOFF`, so it needs CT-2's kill harness or an injectable clock |
| INV-35 | CT-8 | U | |
| INV-36 | CT-5 | P | CT-5 drives each verification switch over a recorded block, forged and honest, and asserts the switch alone decides. The configured upstream batch cap is exercised through both public batch entry points; reduced batches are capped before their first attempt and stay capped while splitting |
| INV-40 | CT-2 | U | |
...
| LIV-2 | CT-4/7 | P | the stall level flips at `P-STALL-ALARM` and CT-4 pins both arms of what makes it decidable: an upstream level with the buffer is idle input however long it sits, an upstream that ran ahead while nothing commits is a stall, and it clears on its own. The persistent-fault corpus and the soak are absent |
...
| LIV-10 | CT-8 | U | |
| LIV-11 | CT-7 | P | CT-7 asserts convergence — a buffer that outgrew its window while finality lagged is back inside it once the reports land — and CT-4 asserts the OB-6 level and magnitude clear on that same commit. The soak is absent |
| LIV-12 | CT-8 | U | |
...
| REQ-16 | CT-4/5/8 | P | Retry classification, including the reduced singleton-batch path, is unit-tested; a partial-batch regression proves an observed successful item is never reissued. Metered regressions pin the batch cap and aggregate concurrent rate, while queued-waiter and fractional-rate regressions exercise atomic and satisfiable reservations. Invalid zero/over-window batch configurations fail startup. Capped chunks overlap under the shared limits and preserve result order. Readiness is a local observation read and contributes no per-probe upstream calls. Full S6 saturation and fairness remain absent |
| REQ-17 | CT-1 | U | the synthetic non-EVM adapter exercises core-managed readiness without an adapter metrics hook; the full skipped-height acceptance run remains absent |
...
| REQ-30..32 | CT-5 | P | REQ-30: the OB signals are present and the family set is pinned to IB-12; CT-5 drives the real adapter against the scripted upstream and asserts OB-4's view and its request/error/retry counters move, WP-11.3 exhaustion included. 12's property→observable table is decidable from the scrape except the SLI rows Phase 4 owns. REQ-31's log-volume half is GAP-23; REQ-32's verification and batch-cap honesty are covered under REQ-14 and INV-36 |
...
Retired rows stay in place — IDs are stable and ADRs cite them: GAP-1, GAP-2, GAP-3, GAP-4, GAP-5, GAP-6, GAP-7, GAP-8, GAP-9, GAP-10, GAP-11, GAP-12, GAP-13, GAP-14, GAP-15, GAP-17, GAP-18, GAP-19, GAP-20, GAP-21, GAP-22, GAP-24, GAP-25, GAP-27, GAP-29, GAP-32, GAP-33, GAP-34, GAP-35, GAP-36.
...
| GAP-2 | **Retired 2026-08-05.** Stride acquisition is bounded by the finalized head on both streams, so every strided range is final and carries its own report; the prober skips what a report already settled and spaces only rounds that settle nothing. The over-window alarm became a level with GAP-4 | — | retired | — |
...
| GAP-4 | **Retired 2026-08-07.** Every alarm this suite names is a level or a counter on the scrape surface: the over-window level with its magnitude, force advances with the height they passed, integrity violations, session restarts split by cause, fork rebases, watermark regressions, acquisition-retry exhaustion, the FM-30 terminal level, and the LIV-2 stall level flipping at `P-STALL-ALARM` with an OB-10 capture on the flip. The stall bound runs from whichever came later, the last commit or the moment the upstream got ahead, so a chain that idled past the threshold does not alarm on the next block it produces. INV-31's row names the one restart cause still without a behavioural test | — | retired | — |
...
| GAP-20 | **Retired 2026-08-07.** Both batch entry points split a logical call list at `max_batch_call_size` before its first round trip; reduce-on-retry can only shrink one of those capped chunks and preserves result order across them. Independent capped chunks overlap without exceeding the same finite `P-RPC-CAPACITY` enforced across requests. Zero caps and caps larger than one otherwise-empty rate window can admit are rejected at construction. The regressions observe the `2, 2, 1` cap shape and bounded concurrent ordered completion, with no oversized attempt | — | retired | — |
| GAP-21 | **Retired 2026-08-07.** Rate admission now checks and reserves capacity in one mutex transition, then releases the state before sleeping, retiring HZ-7. Admission compares the projected rate contribution rather than an item count, so a single call remains satisfiable below one item/s. A deterministic concurrency regression queues eight waiters behind a four-call budget and admits exactly four in the first window; the metered CT-8 HTTP regression independently drives eight simultaneous callers, observes at most four in that window, and sees all calls finish across two windows | — | retired | — |
| GAP-22 | **Retired 2026-08-07.** A continuity break mid-splice is a stream error rather than an assertion inside the response task, so both it and a bounded acquisition failure leave the same trail: the prefix already sent stays valid, the body ends on a record boundary, and the cause is counted by `budget`/`error`/`disconnect`. CT-4 drives both failures after a prefix and reads the counter back | — | retired | — |
...
| GAP-24 | **Retired 2026-08-07.** The worker gauge ADR-3 emptied is removed and enumerated as an ADR-1 divergence, so HC-8 asserts its absence instead of diffing it. CT-5 pins the exposed family set to IB-12 exactly, which is what stops the next one. Process- and runtime-level metrics are scoped out rather than implemented (IB-12) | — | retired | — |
| GAP-25 | **Retired 2026-08-07.** The client reports every round trip, error and retry through a hook the acquisition layer maps onto a closed class set, and both upstream watermarks are recorded with the time they were observed. An exhausted retry is classed by what actually failed, so a paging rule on `timeout` does not miss timeouts that retried first. CT-5 drives the real adapter against the scripted upstream for the healthy, retried, and exhausted cases | — | retired | — |
...
| GAP-34 | **Retired 2026-08-07.** Readiness compares the buffer against a typed local copy of OB-4's last upstream-head number and returns not-ready when the last successful explicit read is absent or at least `P-STALL-ALARM` old. The core refreshes freshness through the required `DataSource::get_head` contract; committed source batches may advance the number without masking failed reads, so a generic adapter needs no metrics hook. Probe frequency remains local work: an HTTP regression issues eight probes without eight head reads, then observes 503 after refresh failures let the explicit observation expire; CT-1 reaches ready with the synthetic adapter and no manual publication | — | retired | — |
...
4. **Phase 3 — robustness**: CT-4 upstream-fault matrix (GAP-14/15), CT-9 fuzz
   ...
   The GAP-4/22/24/25 observability slice closed next, as one cut: the OB-1..OB-9
   signals IB-12 was missing all landed together because they are only useful
   together — the stall level is OB-2 read against OB-4, and truncation is only
   an alarm because RP-12 forbids signalling it in band. One registry is now
   shared by the acquisition layer and the buffer, so upstream counters and state
   levels reach the same scrape. INV-27, INV-30, INV-31, LIV-2 and REQ-30..32
   flipped U→P; INV-4 and LIV-11 lost their `!`; HC-10 U→P.
   The GAP-20/21/34 upstream-budget slice closed next. Both RPC batch entry points
   enforce the configured cap before sending, execute independent chunks concurrently
   within the finite request capacity, and reject caps an otherwise-empty rate window
   can never admit. Concurrent rate waiters reserve
   capacity atomically without holding their state lock while asleep; projected-rate
   admission also covers fractional limits. Focused and metered regressions pin the cap
   and the shared rolling-window admission. Readiness consumes a typed, core-maintained
   local OB-4 observation, with explicit-read freshness separate from committed
   progress, instead of creating per-probe upstream traffic. INV-36 and REQ-16 lost
   their `!`; ADR-17 moved to
   Accepted.
...
| HC-10 | Observability scraper + quiescence gate | CT-6, 7; INV-30/31 | P | text-exposition scraper plus a poll-until helper in `crates/data-service-core/tests/common`, used by the CT-4 alarm corpus and the CT-5 surface sweep. The quiescence gate (no pending input, no in-flight request, OB-2 stable across two scrapes) is absent, so the soak classes still cannot use it |
