...
impl Metrics {
    ...
    pub fn with_stall_alarm(stall_after: Duration) -> Self {
        ...
        // Buckets straddle one and two slot times of the slower EVM chains on
        // purpose: a skipped slot lands a sample a whole slot above the mode,
        // which is how the two causes of a wide detection gap are told apart.
        let head_interval_ms = Histogram::with_opts(
            ...
            .buckets(vec![
                100.0, 250.0, 500.0, 1000.0, 2000.0, 4000.0, 8000.0, 12000.0, 13000.0, 16000.0,
                24000.0, 36000.0, 60000.0,
            ]),
        )
        .unwrap();
