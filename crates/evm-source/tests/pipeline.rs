...
async fn cassette_handler(
    State(c): State<Arc<Value>>,
            ...
            "trace_replayBlockTransactions" => {
                ...
                for rep in reps.as_array_mut().into_iter().flatten() {
                    if let Some(o) = rep.as_object_mut() {
                        if !want.contains(&"trace") {
                            o.remove("trace");
                        }
                        if !want.contains(&"stateDiff") {
                            o.remove("stateDiff");
                        }
                    }
                }
...
async fn run_cassette(cassette: Value, req: DataRequest) -> NormalizedBlock {
    ...
    map_rpc_block(
        &blocks[0],
        &MappingOptions {
            with_traces: req.traces,
            with_state_diffs: req.state_diffs,
        },
    )
}
...
async fn pipeline_gnosis_statediffs_present() {
    ...
    assert!(
        !sd.is_empty(),
        "statediffs lost on the trace-API path (real gnosis block)"
    );
}
...
async fn pipeline_gnosis_traces_present() {
    ...
    assert!(
        !tr.is_empty(),
        "traces lost under use_trace_api + statediffs (real gnosis block)"
    );
}
...
async fn pipeline_gnosis_keeps_total_difficulty_when_present() {
    ...
    let nb = run_cassette(fixture("gnosis-pipeline.json"), gnosis_req()).await;
    assert!(
        nb.header.total_difficulty.is_some(),
        "totalDifficulty dropped despite RPC providing it"
    );
}
...
fn real_gnosis_replay_deserializes_statediff_and_hash() {
    ...
    assert!(
        replays[0].state_diff.is_some(),
        "real `stateDiff` (camelCase) not deserialized"
    );
    assert!(
        replays[0].transaction_hash.is_some(),
        "real `transactionHash` (camelCase) not deserialized"
    );
    assert!(
        replays[0]
            .trace
...
fn header_omits_total_difficulty_when_rpc_omits_it() {
    ...
    assert!(
        header.total_difficulty.is_none(),
        "totalDifficulty must be None when the RPC omits it"
    );
    let out = serde_json::to_string(&header).unwrap();
    assert!(
        !out.contains("totalDifficulty"),
        "output must omit totalDifficulty when absent"
    );
}
